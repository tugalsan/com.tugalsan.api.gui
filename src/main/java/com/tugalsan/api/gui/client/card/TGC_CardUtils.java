...
public class TGC_CardUtils {
    ...
    public static TGS_Pack3<TextBox, FlowPanel, List<TGC_Card>> create(ScrollPanel optionalScroll, CharSequence optionalTitleHtml, CharSequence urlSearchIcon, TGS_ExecutableType1<List<TGC_Card>> cards) {
        ...
        tbInput.addKeyUpHandler(e -> {
            ...
            var searchTagTrim = TGS_CharSet.toLowerCase_TR(TGS_StringUtils.removeHidden(tbInput.getText().trim()));
            ...
            IntStream.range(0, cardsInner.size()).forEachOrdered(i -> {
                ...
                var contentTrim = TGS_CharSet.toLowerCase_TR(TGS_StringUtils.removeHidden(card.htmlContent.toString().trim()));
